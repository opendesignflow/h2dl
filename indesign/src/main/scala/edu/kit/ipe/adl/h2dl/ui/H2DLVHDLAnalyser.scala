...
import edu.kit.ipe.adl.h2dl.tool.ghdl.GHDLTool

class H2DLVHDLAnalyser extends IndesignUIView {

  var selectedFile: Option[VHDLFile] = None
  ...
  this.viewContent {
    ...
    div {
      ...
      "ui table" :: table {
        ...
        tbody {
          Harvest.onHarvesters[VHDLFileHarvester] {
            case h =>
              h.onResources[VHDLFile] {
                r =>
                  tr {
      ...
      selectedFile match {
        case Some(verilogFileResource) =>
          ...
          p {
            textContent("Using GHDL Verilog, you can compile this verilog file to check for syntax errors")
          }
          div {
            ...
            "ui button" :: button("Compile File") {
              reRender
              onClick {
                var ghdl = GHDLHarvester.getResource[GHDLTool].get

                var vfile = verilogFileResource.path.toFile()
                var outFile = new File(vfile.getAbsolutePath.replace(".v", ".vpp"))

                var workdir = new File("ghdlwork").getAbsoluteFile
                workdir.mkdirs()

                var process = ghdl.createToolProcess("-c", "--workdir=" + workdir.getAbsolutePath, vfile.getAbsolutePath)
          ...
          "ui button" :: button("Run Simulation") {
            ...
            onClick {
              ...
              //-- Args
              var args = Array("-c", "--workdir="+workdir.getAbsolutePath)
              args = (args ++ files) :+ vfile.getAbsolutePath

              println(s"Sim Args: " + args.toList)

              var process = ghdl.createToolProcess(args)
              process.outputToBuffer
              runProcess = Some(process)
